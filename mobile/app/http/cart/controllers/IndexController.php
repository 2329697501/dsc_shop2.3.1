			...
			foreach ($cart_goods['goods_list'] as $k => $list) {
				if ($list['goods_list']) {
					...
					foreach ($list['goods_list'] as $key => $val) {
						...
						if ($cart_goods['goods_list'][$k]['goods_list'][$key]['extension_code']) {
							$sql = 'SELECT activity_thumb FROM {pre}goods_activity WHERE act_id =' . $cart_goods['goods_list'][$k]['goods_list'][$key]['goods_id'] . ' and is_finished = 0';
	...
	public function actionCartBonus()
	{
		...
		if (is_int($ru_id)) {
			...
			foreach ($bonus as $key) {
				...
				if (0 <= $key['type_money']) {
					if (50 <= $key['type_money']) {
						if (100 <= $key['type_money']) {
							$num = 1;
						}
						else {
							$num = 2;
