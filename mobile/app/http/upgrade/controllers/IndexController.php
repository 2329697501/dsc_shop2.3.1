...
class IndexController extends \http\base\controllers\BackendController
{
	...
	public function actionInit()
	{
		...
		foreach ($pathlist as $k => $v) {
			...
			file_put_contents($upgrade_zip, \libraries\Http::curlPost($upgradezip_url));
	...
	public function checkfile()
	{
		...
		if (!empty($do)) {
			$this->ec_readdir('.');
			$ectouch_md5 = \libraries\Http::doGet($this->_upgrademd5 . RELEASE . '_' . $this->patch_charset . '.php');
	...
	private function pathlist()
	{
		...
		$pathlist_str = \libraries\Http::doGet($patch_url);
