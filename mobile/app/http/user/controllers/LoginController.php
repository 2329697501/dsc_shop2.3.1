...
class LoginController extends \http\base\controllers\FrontendController
{
	public $user;
	public $user_id;
	...
	public function actionEditPassword()
	{
		...
		if (isset($_SESSION['user_id']) && (0 < $_SESSION['user_id'])) {
			...
			$this->redirect(u('login', array('referer' => urlencode(u($this->action)))));
		}
	}
		...
		if (IS_POST) {
			...
			if (i('enabled_sms') == 1) {
				...
				if ((intval(c('shop.captcha')) & CAPTCHA_REGISTER) && (0 < gd_version())) {
					...
					$validator = new \ectouch\verify\Verify();
