...
{
	$sql = 'SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order ' . ' FROM ' . $GLOBALS['ecs']->table('category') . ' AS c WHERE c.parent_id = \'' . $parent_id . '\' ' . ' order by c.sort_order, c.cat_id';
...
if ($_REQUEST['act'] == 'move_cat') {
	...
	$sql = 'UPDATE ' . $ecs->table('goods') . ' SET cat_id = \'' . $target_cat_id . '\' ' . 'WHERE cat_id = \'' . $cat_id . '\' AND user_id = \'' . $adminru['ru_id'] . '\'';
