...
if ($_REQUEST['act'] == 'get_select_category') {
	...
	if ($act_type == 'goods_img') {
		...
		$original_img = '';
		$old_original_img = '';
		if ((($_FILES['goods_img']['tmp_name'] != '') && ($_FILES['goods_img']['tmp_name'] != 'none')) || (($_POST['goods_img_url'] != $_LANG['lab_picture_url']) && ($_POST['goods_img_url'] != 'http://') && ($is_url_goods_img = 1))) {
			if (empty($is_url_goods_img)) {
				$original_img = $image->upload_image($_FILES['goods_img']);
			}
			else if (copy(trim($_POST['goods_img_url']), ROOT_PATH . 'temp/' . basename($_POST['goods_img_url']))) {
	...
	$sql = 'SELECT action_id, parent_id, action_code,relevance FROM ' . $ecs->table('admin_action') . ' WHERE parent_id ' . db_create_in($db_create_in);
	$result = $db->query($sql);
