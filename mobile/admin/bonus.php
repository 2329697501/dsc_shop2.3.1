...
if ($_REQUEST['act'] == 'update') {
	...
	$sql = 'UPDATE ' . $ecs->table('bonus_type') . ' SET ' . 'type_name       = \'' . $type_name . '\', ' . 'type_money      = \'' . $_POST['type_money'] . '\', ' . 'send_start_date = \'' . $send_startdate . '\', ' . 'send_end_date   = \'' . $send_enddate . '\', ' . 'use_start_date  = \'' . $use_startdate . '\', ' . 'use_end_date    = \'' . $use_enddate . '\', ' . 'send_type       = \'' . $_POST['send_type'] . '\', ' . 'usebonus_type   = \'' . $usebonus_type . '\', ' . 'min_amount      = \'' . $min_amount . '\', ' . 'min_goods_amount = \'' . floatval($_POST['min_goods_amount']) . '\' ' . 'WHERE type_id   = \'' . $type_id . '\'';
	$db->query($sql);
