...
class qq
{
	...
	public function callback($callback_url, $code)
	{
		...
		if (isset($result['access_token']) && ($result['access_token'] != '')) {
			...
			$data = array('openid' => basename(__FILE__, '.php') . '_' . $openid, 'name' => $this->get_user_name($userinfo), 'sex' => $userinfo['gender'], 'avatar' => $userinfo['figureurl_qq_2'] ? $userinfo['figureurl_qq_2'] : $userinfo['figureurl_qq_1']);
			return $data;
		}
		else {
			return false;
