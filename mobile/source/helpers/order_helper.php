...
function available_payment_list($support_cod, $cod_fee = 0, $is_online = false)
{
	...
	include_once ROOT_PATH . 'source/helpers/compositor.php';
...
function order_fee($order, $goods, $consignee, $type = 0, $cart_value = '', $pay_type = 0, $cart_goods_list = '')
{
	...
	if (($order['extension_code'] == 'group_buy') && (0 < $group_buy['deposit'])) {
		...
		if (($order['extension_code'] == 'presale') && (0 < $presale['deposit'])) {
			...
			if ($total['couman'] < ($total['amount'] - $use_bonus)) {
...
function cart_goods($type = CART_GENERAL_GOODS, $cart_value = '', $ru_type = 0, $warehouse_id = 0, $area_id = 0, $consignee = '')
{
	$rec_txt = array('普通', '团够', '拍卖', '夺宝奇兵', '抵用券商城', '预售');
	...
	$sql = 'SELECT c.warehouse_id, c.area_id, c.rec_id, c.user_id, c.goods_id, g.user_id as ru_id, c.goods_name, g.goods_thumb, c.goods_sn, c.goods_number, g.default_shipping, g.goods_weight as goodsWeight, ' . 'c.market_price, c.goods_price, c.goods_attr, c.is_real, c.extension_code, c.parent_id, c.is_gift, c.is_shipping, c.rec_type, ' . 'c.goods_price * c.goods_number AS subtotal, c.goods_attr_id, c.goods_number, ' . ' c.parent_id, c.group_id, pa.deposit ' . 'FROM ' . $GLOBALS['ecs']->table('cart') . ' AS c ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON c.goods_id = g.goods_id ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('presale_activity') . ' AS pa ON pa.goods_id = g.goods_id ' . 'WHERE ' . $where . ' ' . $c_sess . 'AND rec_type = \'' . $type . '\'' . $goodsIn . ' order by c.rec_id DESC';
	...
	if (($ru_type == 1) && (0 < $warehouse_id)) {
		$leftJoin = ' left join ' . $GLOBALS['ecs']->table('warehouse_goods') . ' as wg on g.goods_id = wg.goods_id and wg.region_id = \'' . $warehouse_id . '\' ';
		$leftJoin .= ' left join ' . $GLOBALS['ecs']->table('warehouse_area_goods') . ' as wag on g.goods_id = wag.goods_id and wag.region_id = \'' . $area_id . '\' ';
		$sql = 'SELECT IF(g.model_price < 1, g.goods_number, IF(g.model_price < 2, wg.region_number, wag.region_number)) AS goods_number, g.user_id, g.model_attr FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . $leftJoin . ' WHERE g.goods_id = \'' . $value['goods_id'] . '\' LIMIT 0, 1';
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0, $warehouse_id = 0, $area_id = 0)
{
	...
	$parent = array('user_id' => $_SESSION['user_id'], 'session_id' => $sess, 'goods_id' => $goods_id, 'goods_sn' => addslashes($goods['goods_sn']), 'product_id' => $product_info['product_id'], 'goods_name' => addslashes($goods['goods_name']), 'market_price' => $goods['market_price'], 'goods_attr' => addslashes($goods_attr), 'goods_attr_id' => $goods_attr_id, 'is_real' => $goods['is_real'], 'model_attr' => $goods['model_attr'], 'warehouse_id' => $warehouse_id, 'area_id' => $area_id, 'ru_id' => $goods['ru_id'], 'extension_code' => $goods['extension_code'], 'is_gift' => 0, 'is_shipping' => $goods['is_shipping'], 'rec_type' => CART_GENERAL_GOODS, 'add_time' => gmtime());
	$basic_list = array();
	...
	if (0 < $num) {
		$sql = 'SELECT goods_number FROM ' . $GLOBALS['ecs']->table('cart') . ' WHERE ' . $sess_id . ' AND goods_id = \'' . $goods_id . '\' ' . ' AND parent_id = 0 AND goods_attr = \'' . $goods_attr . '\' ' . ' AND extension_code <> \'package_buy\' ' . ' AND rec_type = \'CART_GENERAL_GOODS\' AND group_id=\'\' AND warehouse_id = \'' . $warehouse_id . '\'';
		$row = $GLOBALS['db']->getRow($sql);
		...
		if ($row) {
			...
			if (($GLOBALS['_CFG']['use_storage'] == 0) || ($num <= $goods_storage)) {
				$goods_price = get_final_price($goods_id, $num, true, $spec, $warehouse_id, $area_id);
				$sql = 'UPDATE ' . $GLOBALS['ecs']->table('cart') . ' SET goods_number = \'' . $num . '\'' . ' , goods_price = \'' . $goods_price . '\'' . ' , area_id = \'' . $area_id . '\'' . ' WHERE ' . $sess_id . ' AND goods_id = \'' . $goods_id . '\' ' . ' AND parent_id = 0 AND goods_attr = \'' . $goods_attr . '\' ' . ' AND extension_code <> \'package_buy\' ' . ' AND warehouse_id = \'' . $warehouse_id . '\' ' . 'AND rec_type = \'CART_GENERAL_GOODS\' AND group_id = 0';
...
function user_bonus($user_id, $goods_amount = 0, $cart_value = 0)
{
	...
	$sql = 'SELECT t.type_id, t.type_name, t.type_money, t.user_id, t.usebonus_type, t.use_start_date, t.use_end_date, b.bonus_id ' . 'FROM ' . $GLOBALS['ecs']->table('bonus_type') . ' AS t,' . $GLOBALS['ecs']->table('user_bonus') . ' AS b ' . 'WHERE t.type_id = b.bonus_type_id ' . 'AND t.use_start_date <= \'' . $today . '\' ' . 'AND t.use_end_date >= \'' . $today . '\' ' . 'AND t.min_goods_amount <= \'' . $goods_amount . '\' ' . 'AND b.user_id<>0 ' . 'AND b.user_id = \'' . $user_id . '\' ' . 'AND b.order_id = 0' . $where;
	return $GLOBALS['db']->getAll($sql);
}
...
function order_refund($order, $refund_type, $refund_note, $refund_amount = 0)
{
	...
	if ($refund_note) {
		...
		include_once ROOT_PATH . 'source/languages/' . c('shop.lang') . '/admin/order.php';
...
function get_ru_shippng_info($cart_goods, $cart_value, $ru_id, $consignee = '')
{
	...
	foreach ($cart_goods as $cgk => $cgv) {
		...
		else if (in_array($cgv['rec_id'], explode(',', $cart_value))) {
...
function exist_real_goods($order_id = 0, $flow_type = CART_GENERAL_GOODS)
{
	...
	if ($order_id <= 0) {
		$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('cart') . ' WHERE ' . $sess_id . ' AND is_real = 1 ' . 'AND rec_type = \'' . $flow_type . '\'';
	}
	else {
...
{
	if ($order['extension_code'] == 'group_buy') {
		include_once ROOT_PATH . 'source/helpers/goods_helper.php';
