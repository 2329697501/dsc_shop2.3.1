...
function get_warehouse_id_attr_number($goods_id, $attr_id = '', $admin_id, $warehouse_id, $area_id, $model_attr = '')
{
	...
	if (empty($attr_id)) {
		...
		foreach ($attr_arr as $key => $val) {
			$sql = ' select a.attr_type from ' . $GLOBALS['ecs']->table('goods_attr') . ' as ga ' . ' left join ' . $GLOBALS['ecs']->table('attribute') . ' as a on a.attr_id=ga.attr_id ' . ' where goods_attr_id=\'' . $val . '\' ';
	...
	foreach ($newInfo as $key => $info) {
		...
		do {
			...
			if (($GLOBALS['_CFG']['sms_order_placed'] == '1') && ($sms_shop_mobile != '')) {
				...
				send_sms($sms_shop_mobile, sprintf($msg, $row['consignee'], $row['mobile']), $sms_type);
			}
		} while ($error_no == 1062);
...
{
	$sql = 'select mc.cat_id, c.cat_name, c.sort_order as vieworder from ' . $GLOBALS['ecs']->table('merchants_category') . ' as mc ' . ' left join ' . $GLOBALS['ecs']->table('category') . ' as c on mc.cat_id = c.cat_id ' . ' where mc.user_id = \'' . $ru_id . '\' and mc.is_show = 1';
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();
...
{
	$sql = 'select c.cat_id, c.cat_name from ' . $GLOBALS['ecs']->table('merchants_category') . ' as mc ' . ' left join ' . $GLOBALS['ecs']->table('category') . ' as c on mc.cat_id = c.cat_id ' . ' where c.parent_id = \'' . $parent_id . '\' and mc.user_id = \'' . $ru_id . '\'';
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();
...
function goodsShippingFee($goods_id = 0, $warehouse_id = 0, $region = array())
{
	...
	if ($is_shipping) {
		if ($goodsInfo['is_shipping'] == 1) {
			...
			$free_money = $goods_shipping_fee['free_money'];
			$shippingInfo['free_money'] = $free_money;
