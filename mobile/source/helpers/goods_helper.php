...
function get_child_treee($tree_id = 0)
{
	...
	if ($GLOBALS['db']->getOne($sql) || ($tree_id == 0)) {
		$child_sql = 'SELECT c.cat_id, c.cat_name, c.parent_id, c.is_show, g.goods_thumb ' . ' FROM ' . $GLOBALS['ecs']->table('category') . ' c' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' g ON c.cat_id = g.cat_id AND g.is_on_sale = 1 AND g.is_delete = 0' . ' WHERE c.parent_id = \'' . $tree_id . '\' AND c.is_show = 1 GROUP BY c.cat_id ORDER BY c.sort_order ASC, c.cat_id ASC';
		$res = $GLOBALS['db']->getAll($child_sql);
...
{
	$group_buy_id = intval($group_buy_id);
	$sql = 'SELECT goods_id ' . 'FROM ' . $GLOBALS['ecs']->table('goods_activity') . 'WHERE act_id = \'' . $group_buy_id . '\' ' . 'AND act_type = \'' . GAT_GROUP_BUY . '\'';
...
{
	$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('favourable_activity') . ' WHERE act_id = \'' . $act_id . '\'';
	$row = $GLOBALS['db']->getRow($sql);
			...
			foreach ($res as $row) {
				...
				if ($two_goods == 1) {
					...
					foreach ($goods_res as $goods_idx => $goods_row) {
						...
						if (0 < $row['market_price']) {
							$discount_arr = get_discount($goods_row['goods_id']);
						}

						$goods[$goods_idx]['zhekou'] = $discount_arr['discount'];
						$goods[$goods_idx]['jiesheng'] = $discount_arr['jiesheng'];
...
function get_child_tree_goods($tree_id = 0, $limit = 6)
{
	...
	if ($GLOBALS['db']->getOne($sql) || ($tree_id == 0)) {
		...
		foreach ($res as $row) {
			...
			$three_arr[$row['cat_id']]['url'] = build_uri('category', array('cid' => $row['cat_id']), $row['cat_name']);
			$children = get_children($row['cat_id']);
			$sql = 'SELECT g.goods_id,g.cat_id, g.goods_name, g.comments_number,g.sales_volume,g.market_price, g.shop_price AS org_price, ' . 'IFNULL(mp.user_price, g.shop_price * \'' . $_SESSION['discount'] . '\') AS shop_price, ' . 'g.promote_price, promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img ' . 'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' . 'ON mp.goods_id = g.goods_id AND mp.user_rank = \'' . $_SESSION['user_rank'] . '\' ' . 'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND ' . 'g.is_delete = 0 AND (' . $children . 'OR ' . get_extension_goods($children) . ') ' . $tag_wher . ' ORDER BY g.sort_order, g.goods_id DESC LIMIT ' . $limit;
			$goods_res = $GLOBALS['db']->getAll($sql);
			$goods = array();
