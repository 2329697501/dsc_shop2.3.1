...
function send_sms($mobile, $content, $sms_type = 'Ihuyi', $send_time = '')
{
	...
	if (!empty($row)) {
		$config = array(
			'driver'       => 'Sms',
			'driverConfig' => array('sms_type' => $sms_type, 'sms_name' => $GLOBALS['_CFG']['sms_ecmoban_user'], 'sms_password' => $GLOBALS['_CFG']['sms_ecmoban_password'], 'SmsCdoe' => $row['temp_id'], 'SignName' => $row['set_sign'])
			);
	}
	else {
		$config = array(
			'driver'       => 'Sms',
			'driverConfig' => array('sms_type' => $sms_type, 'sms_name' => $GLOBALS['_CFG']['sms_ecmoban_user'], 'sms_password' => $GLOBALS['_CFG']['sms_ecmoban_password'])
			);
	}

	$obj = new \libraries\Send($config);

	if ($obj->push($mobile, '', $content)) {
		return true;
	}
	else {
		return $obj->getError();
...
function visit_stats()
{
	...
	$sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('stats') . ' ( ' . 'ip_address, visit_times, browser, system, language, area, ' . 'referer_domain, referer_path, access_url, access_time' . ') VALUES (' . '\'' . $ip . '\', \'' . $visit_times . '\', \'' . $browser . '\', \'' . $os . '\', \'' . $lang . '\', \'' . $area . '\', ' . '\'' . htmlspecialchars(addslashes($domain)) . '\', \'' . htmlspecialchars(addslashes($path)) . '\', \'' . htmlspecialchars(addslashes(PHP_SELF)) . '\', \'' . $time . '\')';
	$GLOBALS['db']->query($sql);
}
...
function show_message($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = true)
{
	...
	$GLOBALS['smarty']->display('source/apps/base/views/message');
	exit();
}
...
function set_affiliate()
{
	...
	if (!empty($_GET['u']) && ($config['on'] == 1)) {
		if (!empty($config['config']['expire'])) {
			...
			setcookie($cookiekey, intval($_GET['u']), gmtime() + (3600 * 24));
		}
	}
}
