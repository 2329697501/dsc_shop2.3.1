...
function cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true, $web_type = '', $cat_type = 0, $goods_user = 0, $admin_user = 0, $array_type = 0)
{
		...
		if ($data === false) {
			...
			foreach ($res as $k => $v) {
				if ($web_type == 'admin') {
					...
					$sql = 'select user_id from ' . $GLOBALS['ecs']->table('merchants_category') . ' where cat_id = \'' . $v['cat_id'] . '\'';
	...
	$options = get_user_category($options, $shopMain_category, $ru_id, $admin_type);
...
{
	$children = (0 < $cat ? '1 AND ' . get_children($cat) : 1);
	$sql = 'SELECT b.brand_id,COUNT(*) AS goods_num ' . 'FROM ' . $GLOBALS['ecs']->table('brand') . 'AS b ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('link_brand') . 'AS lb ON lb.brand_id = b.brand_id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('merchants_shop_brand') . 'AS msb ON msb.bid = lb.bid AND msb.audit_status = 1 ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.brand_id = b.brand_id OR g.brand_id = msb.bid ' . 'WHERE ' . $children . ' AND b.is_show = 1 ' . ' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' . 'GROUP BY b.brand_id HAVING goods_num > 0 limit 0,1';
...
function get_merchants_brands($cat = 0, $app = 'brand')
{
	...
	$sql = 'SELECT b.bid as brand_id, b.brandName as brand_name, b.brandLogo as brand_logo, b.brand_desc, COUNT(*) AS goods_num, IF(b.brandLogo > \'\', \'1\', \'0\') AS tag ' . 'FROM ' . $GLOBALS['ecs']->table('merchants_shop_brand') . 'AS b, ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . 'WHERE g.brand_id = b.bid ' . $children . ' AND is_show = 1 ' . ' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND b.audit_status = 1 ' . 'GROUP BY b.bid HAVING goods_num > 0 ORDER BY tag DESC, b.sort_order ASC';
	$row = $GLOBALS['db']->getAll($sql);

	foreach ($row as $key => $val) {
		$row[$key]['url'] = build_uri($app, array('cid' => $cat, 'bid' => $val['brand_id'], 'act' => 'merchants_brands'), $val['brand_name']);
		$row[$key]['brand_desc'] = htmlspecialchars($val['brand_desc'], ENT_QUOTES);
	}
...
function get_children($cat = 0, $type = 0, $child_three = 0)
{
	...
	if ($child_three == 1) {
		...
		return $type_cat . db_create_in(array_unique(array_merge(array($cat), array_keys(cat_list($cat, 0, false)))));
	}
}
...
{
	include_once ROOT_PATH . 'source/helpers/code_helper.php';
...
function build_uri($app, $params, $append = '', $page = 0, $keywords = '', $size = 0)
{
	...
	$args = array('cid' => 0, 'gid' => 0, 'bid' => 0, 'acid' => 0, 'aid' => 0, 'mid' => 0, 'urid' => 0, 'ubrand' => 0, 'chkw' => '', 'is_ship' => '', 'sid' => 0, 'gbid' => 0, 'auid' => 0, 'sort' => '', 'order' => '', 'actid' => 0, 'stid' => 0);
...
{
	c('shop.no_picture', __TPL__ . 'img/no_image.png');

	if (strtolower(substr($image, 0, 4)) == 'http') {
		$url = $image;
	}
	else if ($GLOBALS['_CFG']['open_oss'] == 1) {
...
function user_uc_call($func, $params = NULL)
{
	if (isset($GLOBALS['_CFG']['integrate_code']) && ($GLOBALS['_CFG']['integrate_code'] == 'ucenter')) {
		...
		if (!function_exists($func)) {
			include_once ROOT_PATH . 'source/helpers/uc_helper.php';
		}
...
function get_final_price($goods_id, $goods_num = '1', $is_spec_price = false, $spec = array(), $warehouse_id = 0, $area_id = 0, $type = 0, $presale = 0)
{
	...
	$sql = 'SELECT IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * \'' . $discount . '\') AS shop_price, ' . $shop_price . 'IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)) as promote_price, ' . ' g.promote_start_date, g.promote_end_date' . ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ON mp.goods_id = g.goods_id AND mp.user_rank = \'' . $user_rank . '\' ' . $leftJoin . ' WHERE g.goods_id = \'' . $goods_id . '\'' . ' AND g.is_delete = 0';
...
function cate_history($size, $page, $sort, $order, $warehouse_id = 0, $area_id = 0)
{
	...
	if (!empty($_COOKIE['ECS']['list_history'])) {
		...
		$sql = 'SELECT b.brand_name, g.goods_sn, g.brand_id, g.goods_id, g.goods_name, g.user_id, g.goods_thumb, ' . 'IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * \'' . $_SESSION['discount'] . '\') AS shop_price, ' . 'IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)) as promote_price ' . ' FROM ' . $GLOBALS['ecs']->table('goods') . ' as g ' . ' left join ' . $GLOBALS['ecs']->table('brand') . ' as b' . ' on g.brand_id = b.brand_id ' . $leftJoin . 'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' . 'ON mp.goods_id = g.goods_id AND mp.user_rank = \'' . $_SESSION['user_rank'] . '\' ' . ' WHERE ' . $where . ' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 group by g.goods_id  ORDER BY ' . $sort . ' ' . $order;
...
function get_store_shop_list($libType = 0, $keywords = '', $count = 0, $size = 15, $page = 1, $sort = 'shop_id', $order = 'DESC', $warehouse_id = 0, $area_id = 0, $store_province = 0, $store_city = 0, $store_district = 0)
{
	$id = '"';
	...
	$sql = 'SELECT msi.shop_id, msi.user_id, msi.shoprz_brandName, msi.shopNameSuffix ' . $select . ' FROM ' . $GLOBALS['ecs']->table('merchants_shop_information') . ' as msi' . $where_table . ' where ' . $whereShop . ' AND merchants_audit = 1 ORDER BY ' . $sort . ' ' . $order;
	$res = $GLOBALS['db']->selectLimit($sql, $size, ($page - 1) * $size);
	$arr = array();
...
function get_store_shop_count($keywords = '', $sort = 'shop_id', $store_province = 0, $store_city = 0, $store_district = 0)
{
	$whereShop = ' 1 ';
	$where = '1';
	$keywords = mysql_like_quote(trim($keywords));
	...
	$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('merchants_shop_information') . ' AS msi' . $where_table . ' where ' . $whereShop . ' AND merchants_audit = 1';
...
function get_user_bouns_new_list($user_id = 0, $page = 1, $type = 0, $pageFunc = '', $amount = 0)
{
	...
	$sql = 'SELECT  u.bonus_id, u.bonus_sn, u.order_id, u.bind_time, b.type_name, b.type_money,b.min_amount, b.min_goods_amount, b.use_start_date, b.use_end_date ' . ' FROM ' . $GLOBALS['ecs']->table('user_bonus') . ' AS u ,' . $GLOBALS['ecs']->table('bonus_type') . ' AS b' . ' WHERE u.bonus_type_id = b.type_id ' . $uOrder . ' AND u.user_id = \'' . $user_id . '\' ' . $useDate . ' order by u.bonus_id DESC ' . $limit;
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();
...
function model()
{
	return new \apps\base\models\BaseModel();
}
