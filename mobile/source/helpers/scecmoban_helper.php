...
function get_order_pdf_goods($order_id = 0)
{
	...
	foreach ($res as $row) {
		if ($row['is_real'] == 0) {
			$filename = ROOT_PATH . 'plugins/' . $row['extension_code'] . '/languages/common_' . c('shop.lang') . '.php';
...
function get_merchants_navigator($ru_id = 0, $ctype = '', $catlist = array())
{
	...
	foreach ($res as $row) {
		$navlist[$row['type']][] = array('cat_id' => 'cid', 'cat_name' => $row['name'], 'opennew' => $row['opennew'], 'url' => $row['url'], 'ctype' => $row['ctype'], 'cid' => $row['cat_id'], 'vieworder' => $row['vieworder'], 'child' => get_store_category_child($row['cat_id'], $row['ru_id']));
	}
...
function get_user_default($user_id = 0)
{
	...
	include_once ROOT_PATH . 'source/helpers/order_helper.php';
...
function presale_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
	...
	if ($res === NULL) {
		...
		if ($data === false) {
			...
			$res = $GLOBALS['db']->getAll($sql);
			$sql = 'SELECT pa.cid, COUNT(*) AS goods_num ' . 'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('presale_activity') . ' AS pa ON pa.goods_id = g.goods_id ' . ' WHERE is_delete = 0 AND is_on_sale = 0 ' . ' GROUP BY pa.cid';
			$res2 = $GLOBALS['db']->getAll($sql);
			$sql = 'SELECT pc.cid, COUNT(*) AS goods_num FROM ' . $GLOBALS['ecs']->table('presale_cat') . ' AS pc ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('presale_activity') . ' AS pa ON pc.cid = pa.cid ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.goods_id = pa.goods_id ' . 'WHERE g.is_delete = 0 AND g.is_on_sale = 0 GROUP BY pa.cid';
...
function presale_info($presale_id, $current_num = 1, $attr_id = array(), $warehouse_id = 0, $area_id = 0)
{
	...
	$sql = 'SELECT b.*,g.goods_name, g.shop_price, g.user_id, g.goods_id, g.goods_product_tag ' . 'FROM ' . $GLOBALS['ecs']->table('presale_activity') . ' AS b ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON b.goods_id = g.goods_id ' . 'WHERE act_id = \'' . $presale_id . '\' ';
...
{
	$presale_id = intval($presale_id);
	$sql = 'SELECT goods_id ' . 'FROM ' . $GLOBALS['ecs']->table('presale_activity') . 'WHERE act_id = \'' . $presale_id . '\' ';
