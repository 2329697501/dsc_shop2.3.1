...
function get_account_log($user_id, $num, $start, $id)
{
	...
	if (!empty($id)) {
		...
		$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('user_account') . ' WHERE user_id = \'' . $user_id . '\'' . ' AND process_type in (' . implode(',', array(SURPLUS_SAVE, SURPLUS_RETURN)) . ')  ORDER BY add_time DESC';
		$res = $GLOBALS['db']->selectLimit($sql, $num, $start);
	}
...
function get_user_order_comment_list($user_id, $type = 0, $sign = 0, $order_id = 0, $size = 0, $start = 0)
{
	...
	if ($type == 1) {
		$sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('order_goods') . ' AS og ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('order_info') . ' AS oi ON og.order_id = oi.order_id ' . 'LEFT JOIN  ' . $GLOBALS['ecs']->table('goods') . ' AS g ON og.goods_id = g.goods_id ' . 'WHERE og.goods_id = g.goods_id AND oi.user_id = \'' . $user_id . '\' and oi.is_zc_order=0  ' . $where . ' ORDER BY oi.add_time DESC';
		$arr = $GLOBALS['db']->getOne($sql);
	}
	else {
		$sql = 'SELECT og.rec_id, og.order_id, og.goods_id, og.goods_name, oi.add_time, g.goods_thumb, g.goods_product_tag, og.ru_id FROM ' . $GLOBALS['ecs']->table('order_goods') . ' AS og ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('order_info') . ' AS oi ON og.order_id = oi.order_id ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON og.goods_id = g.goods_id ' . 'WHERE og.goods_id = g.goods_id AND oi.user_id = \'' . $user_id . '\' and oi.is_zc_order=0  ' . $where . ' ORDER BY oi.add_time DESC';
...
{
	$sql = 'SELECT c.comment_id, c.comment_rank, c.content, c.id_value, c.order_id, c.user_id, c.goods_tag FROM ' . $GLOBALS['ecs']->table('comment') . ' AS c WHERE c.comment_type = 0 AND c.id_value = \'' . $goods_id . '\' AND c.order_id = \'' . $order_id . '\' AND c.parent_id = 0 AND c.user_id = \'' . $user_id . '\'';
