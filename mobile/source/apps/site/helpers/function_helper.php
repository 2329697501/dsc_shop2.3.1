...
function get_store()
{
	...
	foreach ($store_list as $key => $val) {
		...
		$sql = 'SELECT goods_id,goods_name,goods_thumb,goods_img FROM {pre}goods WHERE user_id = ' . $val['user_id'] . ' AND ' . ' is_on_sale = 1 AND is_alone_sale = 1 AND ' . 'is_delete = 0 ' . $review_goods . ' ORDER BY sort_order,goods_id desc LIMIT 3';
		$goods_list = $GLOBALS['db']->getAll($sql);
