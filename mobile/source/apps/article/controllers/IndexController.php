...
class IndexController extends \apps\base\controllers\FrontendController
{
	...
	public function actionIndex()
	{
		...
		$article_nav = $this->db->query('select cat_id, cat_name from {pre}article_cat where parent_id = 0 order by sort_order,cat_id asc');
	...
	public function actionWechatNewsInfo()
	{
		...
		$data = $this->db->table('wechat_media')->field('title, content')->where(array('id' => $news_id))->find();
