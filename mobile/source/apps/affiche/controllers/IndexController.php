...
namespace apps\affiche\controllers;
		...
		if ($act == 'js') {
			...
			if (!empty($ad_info)) {
				...
				case '0':
					...
					$str = '<a href="' . $url . u('affiche/index', array('ad_id' => $ad_info['ad_id'])) . '&from=' . $_GET['from'] . '&uri=' . urlencode($ad_info['ad_link']) . '" target="_blank">' . '<img src="' . $src . '" border="0" alt="' . $ad_info['ad_name'] . '" /></a>';
					break;
				...
				case 3:
					$str = '<a href="' . u('affiche/index', array('ad_id' => $ad_info['ad_id'], 'from' => $_GET['from'], 'uri' => urlencode($ad_info['ad_link']))) . '" target="_blank">' . nl2br(htmlspecialchars(addslashes($ad_info['ad_code']))) . '</a>';
					break;
				}
			}
