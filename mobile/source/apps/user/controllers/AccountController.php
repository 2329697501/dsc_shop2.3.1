...
class AccountController extends \apps\base\controllers\FrontendController
{
	public $user_id;
	...
	public function actionDeposit()
	{
		...
		foreach ($payment_list as $key => $val) {
			if (!file_exists(ROOT_PATH . 'plugins/payment/' . $val['pay_code'] . '.php')) {
	...
	{
		$bonus = get_user_bouns_list($this->user_id, 15, 0);
		$status['one'] = 0;
		$status['two'] = 0;
		$status['three'] = 0;
