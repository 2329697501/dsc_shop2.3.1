...
function presale_settle_status($extension_id)
{
	...
	if (($row['pay_start_time'] <= $now) && ($now <= $row['pay_end_time'])) {
		$result['start_time'] = local_date('m-d H:i', $row['pay_start_time']);
		$result['end_time'] = local_date('m-d H:i', $row['pay_end_time']);
