...
namespace apps\store\controllers;
...
class IndexController extends \apps\base\controllers\FrontendController
{
	...
	public function actionIndex()
	{
		...
		if (IS_AJAX) {
			...
			if (($lat == 0) && ($lng == 0)) {
				$sql = "SELECT * FROM {pre}merchants_shop_information as a LEFT JOIN {pre}seller_shopinfo as b ON a.user_id=b.ru_id\r\n                    WHERE " . $condition . ' and a.is_street = 1 and a.merchants_audit = 1 order by a.shop_id' . $limit;
			}

			if (!empty($lat) || !empty($lng)) {
				$sql = 'SELECT b.*,a.*,( 6371 * acos( cos( radians(' . $lat . ') ) * cos( radians( b.lat ) ) * cos( radians( b.lng ) - radians(' . $lng . ') ) + sin( radians(' . $lat . ") ) * sin( radians( b.lat ) ) )) AS distance FROM {pre}merchants_shop_information as a LEFT JOIN {pre}seller_shopinfo as b ON a.user_id=b.ru_id\r\n                    WHERE " . $condition . ' and a.merchants_audit = 1 and a.is_street = 1 order by ' . $order . $limit;
			}
	...
	{
		$id = intval($id);
		$info['count_goods'] = $this->sql('user_id=' . $id . '   AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0' . $this->review_goods);
		$info['count_goods_new'] = $this->sql('is_new = 1 AND user_id=' . $id . '   AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0' . $this->review_goods);
		$info['count_goods_promote'] = $this->sql('is_promote = 1 AND user_id=' . $id . '   AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0' . $this->review_goods);
		$info['count_bonus'] = $this->sql('user_id=' . $id . ' AND send_end_date>' . time(), '');
		$info['bonus_all'] = $this->sql('user_id=' . $id . ' AND send_end_date>' . time(), '', 1);
		return $info;
	}
