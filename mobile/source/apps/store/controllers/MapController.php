...
class MapController extends \apps\base\controllers\FrontendController
{
	...
	public function actionIndex()
	{
		if (IS_POST) {
			$lng = i('post.lng', 0);
			$lat = i('post.lat', 0);
			$sql = 'SELECT b.shop_name, b.province, b.city, b.district, b.shop_address, b.lng, b.lat, ( 6371 * acos( cos( radians(' . $lat . ') ) * cos( radians( b.lat ) ) * cos( radians( b.lng ) - radians(' . $lng . ') ) + sin( radians(' . $lat . ') ) * sin( radians( b.lat ) ) ) ) AS distance FROM {pre}seller_shopinfo as b LEFT JOIN {pre}merchants_shop_information as a ON a.user_id=b.ru_id  WHERE a.is_street = 1 and a.merchants_audit = 1 HAVING distance < 100 ORDER BY distance LIMIT 10';
			...
			foreach ($seller_shopinfo as $key => $vo) {
				...
				$info = array('coord' => $vo['lat'] . ',' . $vo['lng'], 'title' => $vo['shop_name'], 'addr' => $address);
	...
	public function actionTest()
	{
		...
		foreach ($seller_shopinfo as $key => $vo) {
			...
			if (!$data['status']) {
				...
				$condition['id'] = $vo['id'];
				$this->model->table('seller_shopinfo')->data($location)->where($condition)->update();
			}
