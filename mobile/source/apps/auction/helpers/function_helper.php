...
function auction_list($keywords, $sort, $order, $size, $page)
{
	...
	$sql = 'SELECT a.*, IFNULL(g.goods_thumb, \'\') AS goods_thumb ' . 'FROM ' . $GLOBALS['ecs']->table('goods_activity') . ' AS a ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON a.goods_id = g.goods_id ' . 'WHERE a.act_type = \'' . GAT_AUCTION . '\' ' . $where . 'AND a.start_time <= \'' . $now . '\' AND a.end_time >= \'' . $now . '\' AND a.is_finished < 2 ORDER BY ' . $sort . ' ' . $order;
