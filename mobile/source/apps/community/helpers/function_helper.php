...
function community_list($type = 0, $page = 1, $size = 10, $user_id = '')
{
	...
	$sql = 'SELECT d.*, u.user_name, u.user_picture, g.goods_name FROM {pre}discuss_circle d ' . ' LEFT JOIN {pre}users u ON d.user_id = u.user_id ' . ' LEFT JOIN {pre}goods g ON d.goods_id = g.goods_id ' . ' WHERE d.parent_id = 0 AND d.user_id <> 0 AND d.dis_type != 4' . $where . ' ORDER BY add_time DESC LIMIT ' . (($page - 1) * $size) . ',  ' . $size;
...
function sd_count($user_id = '')
{
	...
	$sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('comment') . ' AS cmt ' . 'LEFT JOIN (SELECT comment_id,goods_id,comment_img FROM ' . $GLOBALS['ecs']->table('comment_img') . ' GROUP BY comment_id) cmt2 ON (cmt2.comment_id = cmt.comment_id) ' . 'WHERE cmt2.comment_img != \'\'' . $where;
