...
function favourable_list($user_rank, $user_id = -1, $fav_id = 0)
{
	...
	$user_rank = ',' . $user_rank . ',';
	$now = gmtime();
	$sql = 'SELECT * ' . 'FROM ' . $GLOBALS['ecs']->table('favourable_activity') . ' WHERE CONCAT(\',\', user_rank, \',\') LIKE \'%' . $user_rank . '%\'' . ' AND start_time <= \'' . $now . '\' AND end_time >= \'' . $now . '\' ' . $where . ' ORDER BY sort_order';
...
function get_act_range_ext($user_rank, $user_id = -1, $act_range)
{
	...
	$user_rank = ',' . $user_rank . ',';
	$now = gmtime();
	$sql = 'SELECT act_range_ext ' . 'FROM ' . $GLOBALS['ecs']->table('favourable_activity') . ' WHERE CONCAT(\',\', user_rank, \',\') LIKE \'%' . $user_rank . '%\'' . ' AND start_time <= \'' . $now . '\' AND end_time >= \'' . $now . '\' ' . $u_id . $a_range . ' ORDER BY sort_order';
