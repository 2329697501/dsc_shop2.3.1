...
{
	$sql = ' SELECT oi.user_id,oi.add_time,u.user_name,u.user_picture,zg.price,zg.content ' . ' FROM {pre}order_info as oi ' . ' LEFT JOIN {pre}users as u on u.user_id=oi.user_id ' . ' LEFT JOIN {pre}zc_goods as zg on zg.id=oi.zc_goods_id ' . ' LEFT JOIN {pre}zc_project as zd on zd.id=zg.pid ' . ' WHERE oi.is_zc_order=1 AND oi.pay_status=2 AND zd.id = \'' . $goods_id . '\' ' . ' ORDER BY oi.order_id DESC ';
...
function zc_get_order_post_shipping($shipping)
{
	if ($shipping) {
		$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('shipping') . ' WHERE shipping_id = \'' . $shipping . '\' ' . 'AND enabled = 1';
...
function zc_order_fee($order, $goods, $consignee)
{
	...
	$shopping = zc_get_order_post_shipping($order['shipping_id']);
	$total['shipping_fee'] = $shopping['shipping_fee'];
