...
namespace apps\topic\controllers;
...
class IndexController extends \apps\base\controllers\FrontendController
{
	...
	public function actionIndex()
	{
		...
		if (IS_AJAX) {
			$sql = 'SELECT COUNT(*) as num FROM ' . $GLOBALS['ecs']->table('topic') . ' WHERE ' . gmtime() . ' >= start_time and ' . gmtime() . '<= end_time ';
			$num = $GLOBALS['db']->getOne($sql);
			$num ? $num : 0;
			$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('topic') . ' WHERE ' . gmtime() . ' >= start_time and ' . gmtime() . '<= end_time ' . ' ORDER BY start_time DESC ' . ' LIMIT ' . (($page - 1) * $size) . ',  ' . $size;
	...
	public function actionDetail()
	{
		...
		$sql = 'SELECT topic_id FROM ' . $GLOBALS['ecs']->table('topic') . 'WHERE topic_id = \'' . $topic_id . '\' and  ' . gmtime() . ' >= start_time and ' . gmtime() . '<= end_time';
