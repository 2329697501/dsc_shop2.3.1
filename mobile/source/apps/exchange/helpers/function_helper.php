...
function exchange_get_goods($children, $min, $max, $ext, $size, $page, $sort, $order)
{
	...
	$sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, eg.exchange_integral, ' . 'g.goods_type, g.goods_brief, g.goods_thumb , g.goods_img, eg.is_hot ' . $select . 'FROM ' . $GLOBALS['ecs']->table('exchange_goods') . ' AS eg, ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . 'WHERE eg.goods_id = g.goods_id AND ' . $where . ' ' . $ext . '  AND g.is_on_sale = 1 ORDER BY ' . $sort . ' ' . $order;
