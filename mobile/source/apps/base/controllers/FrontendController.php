...
abstract class FrontendController extends BaseController
{
	...
	private function getTpl($tpl = '', $isTpl = false)
	{
		if ($isTpl) {
			...
			$base_themes = ROOT_PATH . 'themes/' . c('TPL.TPL_TEMPLATE') . '/';
			$extends_tpl = APP_NAME . c('TPL.TPL_DEPR') . $tpl . c('TPL.TPL_SUFFIX');

			if (file_exists($base_themes . $extends_tpl)) {
				$tpl = 'themes/' . c('TPL.TPL_TEMPLATE') . '/' . APP_NAME . c('TPL.TPL_DEPR') . $tpl;
			}
			else {
	...
	private function init()
	{
		...
		l(require BASE_PATH . 'language/' . c('shop.lang') . '/common.php');
	...
	private function init_first()
	{
		...
		$init_cache = ROOT_PATH . 'data/caches/app/' . APP_NAME . '/';
