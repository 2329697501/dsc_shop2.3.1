...
class MysqlPdoDriver implements DbInterface
{
	...
	public function query($sql, array $params = array())
	{
		...
		\base\Hook::listen('dbQueryBegin', array($sql, $params));

		if ($sth->execute()) {
			$data = $sth->fetchAll(\PDO::FETCH_ASSOC);
			\base\Hook::listen('dbQueryEnd', array($this->getSql(), $data));
			return $data;
		}

		$err = $sth->errorInfo();
		\base\Hook::listen('dbException', array($this->getSql(), $err[2]));
	...
	public function execute($sql, array $params = array())
	{
		...
		\base\Hook::listen('dbExecuteBegin', array($sql, $params));

		if ($sth->execute()) {
			$affectedRows = $sth->rowCount();
			\base\Hook::listen('dbExecuteEnd', array($this->getSql(), $affectedRows));
			return $affectedRows;
		}
